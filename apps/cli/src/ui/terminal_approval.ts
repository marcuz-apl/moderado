...
export class TerminalApprovalHandler implements IApprovalHandler {
  ...
  async requestApproval(
    ...
    this.stdout.write('\n\x1b[1;33m─────────────────────────────────────────────────────────────\x1b[0m\n');
    this.stdout.write(`\x1b[1;33m[APPROVAL REQUIRED]\x1b[0m Action: \x1b[1m${request.toolName}\x1b[0m\n`);

    if (request.exactPayload.targetFile) {
      this.stdout.write(`\x1b[36mTarget File:\x1b[0m ${request.exactPayload.targetFile}\n`);
    }

    if (request.exactPayload.diffPreview) {
      this.stdout.write(`\x1b[36mDiff Preview:\x1b[0m\n`);
      for (const line of request.exactPayload.diffPreview.split('\n')) {
        if (line.startsWith('+')) {
          this.stdout.write(`\x1b[32m${line}\x1b[0m\n`);
        } else if (line.startsWith('-')) {
          this.stdout.write(`\x1b[31m${line}\x1b[0m\n`);
        } else {
    ...
    if (request.exactPayload.command) {
      this.stdout.write(`\x1b[36mCommand:\x1b[0m ${request.exactPayload.command.join(' ')}\n`);
      if (request.exactPayload.cwd) {
        this.stdout.write(`\x1b[36mWorking Dir:\x1b[0m ${request.exactPayload.cwd}\n`);
      }
    }

    this.stdout.write('\x1b[1;33m─────────────────────────────────────────────────────────────\x1b[0m\n');

    const answer = await this.prompt('Approve this action? [y/N/q]: ', signal);

...
- [x] Write failing tests for `SourcePositionSchema` with one-based coordinates and `SourceLocationSchema` with optional bounded preview.
- [x] Run `npm test -- packages/contracts/tests/contracts.test.ts` and confirm failure.
- [x] Add Zod schemas and `GetDefinitionParamsSchema` / `FindReferencesParamsSchema`.
...
- [x] Write failing fake-transport tests that assert initialize, didOpen, definition/references request order; reject malformed Content-Length framing; and discard locations outside the workspace.
- [x] Run `npm test -- packages/tools/tests/lsp.test.ts` and confirm failure.
- [x] Implement `requestLanguageServer(root, executable, method, position)` using fixed `spawn(executable, ['--stdio'])`, JSON-RPC framing, a 1 MB message cap, 10-second initialization timeout, and process cleanup in `finally`.
- [x] Re-run the focused test.
...
- [x] Write failing tests for missing configured executable, valid workspace result mapping, and both registry declarations.
- [x] Run focused tests and confirm failure.
- [x] Implement read-only tools that request configured language-server executable, resolve requested and returned paths through the jail, and return bounded locations with source previews.
- [x] Register both tools and rerun focused tests.
...
- [x] Add optional `typescriptLanguageServer` configuration and document manual installation/configuration.
- [x] Run `npm test; npm run build; git diff --check`.
- [x] Commit with `git commit -m "feat(tools): add TypeScript language intelligence"`.
